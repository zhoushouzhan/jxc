<template>
    <yplayout>
        <template #header="head">
            <i class="ri-2x ri-building-3-fill text-gray-500 ml-2"></i>
...
<script setup>
    ...
    const getList=async ()=>{
        let resp= await getData('factory/index',{page:pageData.currentPage})
        if(resp.code==1){
    ...
    const removeItem = (id) => {
        ...
        confirms({ text: '确认删除吗？' })
            .then(async () => {
            const res = await postData('factory/delete', {
                ids: id
            })
            if (res.code) {
